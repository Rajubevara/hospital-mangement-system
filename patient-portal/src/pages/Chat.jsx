...
const Chat = () => {
  ...
  return (
    <div className="h-[calc(100vh-10rem)] bg-slate-900 border border-slate-850 rounded-3xl overflow-hidden shadow-xl flex">
      {/* Sidebar List */}
      <div className="w-80 border-r border-slate-855 flex flex-col bg-slate-900/50">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
          {loading ? (
            ...
            <p className="text-center text-xs text-slate-500 py-10 italic">No consulting doctors active.</p>
          ) : (
              ...
              <button
                ...
                className={`w-full p-3 rounded-2xl flex items-center gap-3 text-left border cursor-pointer transition-all ${
                  activePartner?.userId === partner.userId
                    ? 'bg-blue-650/10 text-blue-400 border-blue-500/25 shadow-inner'
                    : 'text-slate-400 hover:bg-slate-850/40 hover:text-slate-205 border-transparent'
                }`}
              >
                <div className="h-9 w-9 bg-slate-950 border border-slate-850 rounded-xl flex items-center justify-center text-blue-400">
                  <Stethoscope size={16} />
                </div>
                <div className="min-w-0 flex-1">
                  <h4 className="font-bold text-sm text-slate-200 truncate">{partner.name}</h4>
                  <p className="text-[10px] text-slate-500 truncate">{partner.specialty}</p>
                </div>
              </button>
            ))
          )}
      ...
      {/* Message Feed Area */}
      <div className="flex-1 flex flex-col justify-between bg-slate-955/20">
        {activePartner ? (
          <>
            {/* Header info */}
            <div className="p-4 border-b border-slate-850 bg-slate-900/60 flex items-center gap-3">
              <div className="h-9 w-9 bg-slate-955 border border-slate-850 rounded-xl flex items-center justify-center text-blue-400">
                <Stethoscope size={16} />
              </div>
              <div>
                <h4 className="font-bold text-white text-sm">{activePartner.name}</h4>
                <p className="text-[10px] text-slate-500">{activePartner.specialty}</p>
              </div>
            </div>
                ...
                return (
                  <div key={msg._id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-md rounded-2xl p-4 shadow ${
                      ...
                      <span className="text-[9px] block text-right text-blue-200/60 mt-1.5 flex items-center justify-end gap-1 font-mono">
                        <Clock size={10} />
            ...
            <form onSubmit={handleSend} className="p-4 border-t border-slate-850 bg-slate-900/40 flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-955 border border-slate-850 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500 rounded-2xl py-3 px-4 text-slate-200 placeholder-slate-650 outline-none"
              />
              <button
                type="submit"
                className="bg-blue-600 hover:bg-blue-500 text-white p-3.5 rounded-2xl shadow-lg shadow-blue-500/10 cursor-pointer transition-colors"
              >
                <Send size={18} />
              </button>
            </form>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-550 space-y-3">
            <MessageSquare size={36} className="text-slate-700" />
            <p className="text-sm italic">Select a clinical practitioner from the side panel to start chat.</p>
          </div>
        )}
      </div>
    </div>
